...
def app():
    ...
    adwordsbyhour_data['Hora'] = pd.to_datetime(adwordsbyhour_data['Hora'], format='%H:%M:%S')
    adwordsbyhour_data['Hora'] = adwordsbyhour_data['Hora'].dt.hour
    adwordsbyhour_data= adwordsbyhour_data.sort_values(by='Hora', ascending=True)
    ...
    adwords1_data['Fecha'] = pd.to_datetime(adwords1_data['Fecha'], format='%Y-%m-%d')
    mask = (adwords1_data['Fecha'] > start_date) & (adwords1_data['Fecha'] <= end_date)
    ...
    end_date=np.datetime64(end_date)
    adwords3_data['Fecha'] = pd.to_datetime(adwords3_data['Fecha'], format='%Y-%m-%d')
    mask = (adwords3_data['Fecha'] > start_date) & (adwords3_data['Fecha'] <= end_date)
    adwords3_data = adwords3_data.loc[mask]
    adwords3_data=adwords3_data[~adwords3_data['URL del emplazamiento'].astype(str).str.startswith('mobileapp::')]
    ...
    url=[adwords3_data[i] for i in adwords3_data.columns if 'URL del emplazamiento' in i]
    df_url = pd.concat(url).reset_index()
    df_url=df_url.rename(columns={0:'Urls'})
    rurl=df_url.groupby(by=['index','URL del emplazamiento']).count().reset_index()
    rulr=rurl.groupby('URL del emplazamiento').agg('count').sort_values('index', ascending=False).head(10)
    ...
    adwords2_data['Fecha'] = pd.to_datetime(adwords2_data['Fecha'], format='%Y-%m-%d')
    mask = (adwords2_data['Fecha'] > start_date1) & (adwords2_data['Fecha'] <= end_date1)
